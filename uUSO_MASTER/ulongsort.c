...
//#pragma OT(0,Speed) 
 PT_THREAD(ulongsort_process(struct pt *pt))
 {
   ...
   wdt_count[Sort_Process].process_state=RUN;
...
//		}
	  ...
	  //--------------сортировка пузырьком--------------------------------
	   ...
	   //-----------находим медиану--------------------
		...
		{
 			channels[USO_CHANNEL].channel_data_calibrate=GetCalibrateVal(USO_CHANNEL,channels[USO_CHANNEL].channel_data);	//калиброванная величина
  		}

		adc_channels[USO_CHANNEL].new_measuring=0;
		wdt_count[Sort_Process].count++;
		PT_RESTART(pt);
	  //----------------------------------------------	
   }
   PT_END(pt);
